...
access(all) contract EVMHelper{
    ...
    access(all) attachment Poeternal for EVM.CadenceOwnedAccount {
        ...
        access(EVM.Owner) fun mint(to: EVM.EVMAddress, newTokenId: UInt256, name: String, lines: [String;4], author:String , source: String, colour:String): UInt256? {
            return self.call("mint(address,string,string[4],string,string,string)", [Type<UInt256>()], [to, newTokenId, lines, author, source, colour]) as? UInt256
        }
        ...
        access(EVM.Owner) fun safeTransferFrom(from: EVM.EVMAddress, to: EVM.EVMAddress, tokenId: UInt256) {
            self.call("safeTransferFrom(address,address,uint256)", [], [from, to, tokenId])
        }
